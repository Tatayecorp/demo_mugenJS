		...
		ressource.prototype = {
			...
			load : function() {
				...
				return new Promise( function( resolveAll, reject ) {
					...
					// Load DEF
					fetch( ressource.path + '/' + ressource.name + '/' + ressource.name + '.def' )
					...
					.then( function( data ) {
						...
						var pAIR = new Promise( function( resolve, reject ) { 
							...
							fetch( ressource.path + '/' + ressource.name + '/' + data.files.anim )
							.then( function( response ) { 
								return response.text();
							} )
							.then( function( text ){ 
								ressource.AIR = decodeAIR( text );
								resolve(); 
							} )
						...
						var pSFF = new Promise( function( resolve, reject ) { 
							...
							fetch( ressource.path + '/' + ressource.name + '/' + data.files.sprite )
							.then( function( response ) { 
								return response.arrayBuffer();
							} )
							.then( function( arrayBuffer ){ 
								ressource.SFF = decodeSFF( arrayBuffer );
								resolve(); 
							} )
						...
						var pACT = new Promise( function( resolve, reject ) { 
							...
							fetch( ressource.path + '/' + ressource.name + '/' + data.files.pal1 )
							.then( function( response ) { 
								return response.arrayBuffer();
							} )
							.then( function( arrayBuffer ){ 
								ressource.ACT.push( decodeACT( arrayBuffer ) );
								resolve(); 
							} )
